...
import { ChannelListResponse, useChannelList } from '../../Subscriptions/hooks/useChannelList';
import { ChannelSettings } from '../../ChannelPage/ChannelSettingsDialog';
import { useChannelSettings } from '../../Settings/hooks/useChannelSettings';
...
export const CoreSettingsSection: React.FC<CoreSettingsSectionProps> = ({
	...
	const [isFlatFoldersActive, setIsFlatFoldersActive] = useState(false);
	...
	const { total,totalPages, channels } = useChannelList(channelListParams);

	const updateFlatFolderSetting = async  (channelId: string) => {
		await fetch(`/api/channels/${channelId}/settings`, {
			...
			body: JSON.stringify({ skip_video_folder: isFlatFoldersActive})
		});
	}

	const handleChangeFlatFolderSetting = () => {
		console.log(total, totalPages);
		setIsFlatFoldersActive(!isFlatFoldersActive);
		channels.forEach(async (channel) => console.log(channel.channel_id!))
		setPage(page + 1);
	}
	...
	return (
		<ConfigurationCard
		title="Core Settings Ingelbingel"
		>
	...
	control={
		<Switch
		checked={isFlatFoldersActive}
		onChange={() => handleChangeFlatFolderSetting()}
		color="primary"
		/>
	}
